...
export const CartProvider = ({ children }) => {
      ...
      return items.map((item) => ({
        ...
        image: item.image
          ? item.image.startsWith("http")
            ? item.image
            : `${BACKEND_URL}${item.image.startsWith("/") ? "" : "/"}${
                item.image
              }`
          : "/placeholder.jpg",
      }));
  ...
  const removeItem = useCallback(
    async (productId) => {
      if (
        !cartRef.current ||
        !cartRef.current.items.some((i) => i.productId === productId)
      ) {
        return;
      }
  ...
  const value = useMemo(
    ...
    [rawCart, summary, appliedVouchers, loading, actionLoading]
  );
